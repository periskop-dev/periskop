  ...
  const renderNavItems = () => {
    if (props.errors.length === 0) {
      ...
      return props.errors.map((error, index) => {
        return (
          <ListGroup.Item action className={"sidebar-item" + " " + sidebarItemClass(error)} onClick={_ => props.handleErrorSelect(encodeURIComponent(error.aggregation_key))} active={ props.activeError === undefined ? false : error.aggregation_key === props.activeError.aggregation_key } key={index}>
